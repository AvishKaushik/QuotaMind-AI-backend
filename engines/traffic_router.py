...
class TrafficRouter:
    ...
    async def _quota_percent_used(self) -> float:
        try:
            ...
            daily_limit = await cache.get_client().get("quota:daily_token_limit")
    ...
    async def _budget_percent_used(self) -> float:
        try:
            ...
            daily_budget_config = await cache.get_client().get("quota:daily_budget_usd")
            daily_budget = float(daily_budget_config or get_float_env("DAILY_BUDGET_USD", 500.0))
